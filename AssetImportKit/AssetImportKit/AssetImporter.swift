...
public struct AssetImporter {
    ...
    mutating func makeSCNNode(fromAssimpNode aiNode: aiNode,
                              in aiScene: aiScene,
                              atPath path: String,
        ...
        if vertexCount > 0 {
            if let nodeGeometry = makeSCNGeometry(from: aiNode,
                                                  in: aiScene,
                                                  atPath: path,
                                                  imageCache: imageCache) {
        ...
        /*
         ...
         ---------------------------------------------------------------------
         */
        boneNames.append(contentsOf: aiNode.getBoneNames(from: aiScene))
        aiNode.getBoneTransforms(in: aiScene).forEach { boneTransforms[$0.0] = $0.1 }
        /*
         ---------------------------------------------------------------------
         Transform
         ---------------------------------------------------------------------
    ...
    mutating func buildSkeletonDatabase(for scene: AssetImporterScene) {
        ...
        uniqueBoneNodes = scene.getBoneNodes(for: uniqueBoneNames)
        
        print("unique bone nodes \(uniqueBoneNodes.count): \(uniqueBoneNodes)")
        
        uniqueBoneTransforms = getTransforms(for: uniqueBoneNames,
                                             from: boneTransforms)
        
        print("unique bone transforms \(uniqueBoneTransforms.count): \(uniqueBoneTransforms)")
        
        skeleton = findSkeletonNode(in: uniqueBoneNodes)
    ...
    func makeSkinner(for aiNode: aiNode,
        ...
        if nBones > 0 {
            ...
            print("Making Skinner for node: \(nodeName) vertices: \(vertexCount) max-weights: \(maxWeights), nBones: \(nBones)")
            
            let boneWeights = makeBoneWeightsGeometrySource(at: aiNode,
                                                            in: aiScene,
                                                            vertexCount: vertexCount,
                                                            maxWeights: maxWeights)
            let boneIndices = makeBoneIndicesGeometrySource(at: aiNode,
                                                            in: aiScene,
                                                            vertexCount: vertexCount,
                                                            maxWeights: maxWeights,
            ...
            if let node = scene.rootNode.childNode(withName: nodeName, recursively: true) {
                ...
                let skinner = SCNSkinner(baseGeometry: node.geometry,
                                         bones: uniqueBoneNodes,
                                         boneInverseBindTransforms: uniqueBoneTransforms as [NSValue],
